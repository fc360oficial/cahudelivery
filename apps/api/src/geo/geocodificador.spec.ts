...
describe('geocodificar', () => {
  it('resolve pela rua e número no Nominatim com precisão endereco', async () => {
    const fetchFn = jest.fn(async () => resposta(200, [{ lat: '-8.1287217', lon: '-34.9380845', address: { city: 'Recife', suburb: 'Ibura' } }]));
  ...
  it('usa town quando não há city (município pequeno)', async () => {
    const fetchFn = jest.fn(async () => resposta(200, [{ lat: '-8.0', lon: '-35.0', address: { town: 'Goiana' } }]));
    expect((await geocodificar(end, fetchFn))?.cidade).toBe('Goiana');
  });
